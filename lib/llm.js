...
class OpenRouterClient {
  ...
  async searchVideos(query, videosWithMetadata) {
    ...
    console.log('[LLM] Limited to', limitedVideos.length, 'most recent videos for LLM analysis');
  ...
  parseSearchResults(content, videosWithMetadata) {
    ...
    try {
      ...
      if (!matches && content.includes('[') && !content.includes(']')) {
        console.log('[LLM] Attempting to fix truncated JSON response');
        try {
          ...
          if (jsonMatch) {
            matches = JSON.parse(jsonMatch[0]);
            console.log('[LLM] Successfully parsed truncated JSON after fixing');
          }
        } catch (parseError) {
