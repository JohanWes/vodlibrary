...
function removeLruEntry(cacheKey) {
  const meta = lruMeta.get(cacheKey);
  if (!meta) {
    return;
  }
  lruMeta.delete(cacheKey);
  cacheStats.size = Math.max(0, cacheStats.size - meta.size);
}

videoCache.on('del', (cacheKey) => {
  removeLruEntry(cacheKey);
});
videoCache.on('flush', () => {
  lruMeta.clear();
  cacheStats.size = 0;
...
function recordAccess(videoId, options = {}) {
...
async function cacheSegmentFromFile(videoId, segmentNumber, filePath, start, end, options = {}) {
  ...
  if (inFlightFills.has(cacheKey)) {
    return inFlightFills.get(cacheKey);
  }

  if (config.enableStats) {
    const accessKey = getAccessCounterKey(videoId, namespace);
    const accessCount = cacheStats.videoAccess[accessKey] || 0;
    if (accessCount < config.popularityThreshold) {
      return false;
    }
  }
  ...
  const fillPromise = readFileSegment(filePath, start, end)
    .then((data) => cacheSegment(videoId, segmentNumber, data, {
      ...options,
      namespace,
      startByte: start,
      endByte: end
    ...
    .finally(() => {
      inFlightFills.delete(cacheKey);
    });

  inFlightFills.set(cacheKey, fillPromise);
  return fillPromise;
}
