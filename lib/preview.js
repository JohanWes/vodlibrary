...
const qualityPresets = {
  ...
  high: { 
    codec: 'libsvtav1', // Fast CPU encoder since GPU not available in WSL
    crf: 28, 
    preset: 8, // Fastest preset for maximum speed
    scale: '1080:-2', 
    bitrate: '600k'
  }
};
...
class PreviewGenerator {
  ...
  async generateSingleClip(videoPath, timestamp, videoHash, forceRegenerate = false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (preset.codec === 'av1_vaapi') {
        ffmpegCommand.outputOptions([
          `-vaapi_device /dev/dri/renderD128`,
          `-vf format=nv12,hwupload,scale_vaapi=${preset.scale.replace(':-2', ':608')}`,
          `-b:v ${preset.bitrate}`,
      ...
      } else if (preset.codec === 'libaom-av1') {
        ffmpegCommand.outputOptions([
          `-b:v ${preset.bitrate}`,
