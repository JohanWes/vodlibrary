...
try {
  ...
  if (process.platform === 'win32') {
    ...
    console.log('Using system ffmpeg for previews');
  }
} catch (error) {
...
const config = {
  ...
  previewDuration: parsePositiveInt(process.env.PREVIEW_DURATION, 10), // seconds
  previewQuality: process.env.PREVIEW_QUALITY || 'high',
  ...
  minVideoDuration: 0, // computed below
  cleanupDays: parsePositiveInt(process.env.PREVIEW_CLEANUP_DAYS, 30)
};

config.minVideoDuration = config.previewDuration + 1;
...
class PreviewGenerator {
  constructor() {
    ...
    this.maxConcurrentGenerations = config.maxConcurrentGenerations;
    this.previewDuration = config.previewDuration;
    this.minVideoDuration = config.minVideoDuration;
  ...
  setupPreviewDirectory() {
    ...
    if (config.previewDir === publicPreviewDir) return;

    try {
      if (fs.existsSync(publicPreviewDir)) return;
  ...
  async generateSingleClip(videoPath, timestamp, videoHash, forceRegenerate = false) {
    ...
    // Try av1_amf (hardware) with software AV1 fallback
    if (preset.codec === 'av1_amf') {
      try {
        ...
        console.log(`Attempting ${preset.codec} encoding for ${outputFilename}`);
        return await attemptEncoding('av1_amf', options);
      } catch (error) {
        console.warn(`AV1 AMF encoding failed for ${outputFilename}, trying software AV1 (libsvtav1). Error: ${error.message}`);
      }
    }

    // Try libsvtav1 (software AV1)
    if (preset.codec === 'libsvtav1' || preset.codec === 'av1_amf') {
      try {
        const svtOptions = [
          `-crf ${preset.crf || 30}`,
        ...
        console.log(`Attempting libsvtav1 encoding for ${outputFilename}`);
        return await attemptEncoding('libsvtav1', svtOptions);
      } catch (svtError) {
        console.warn(`Software AV1 encoding failed for ${outputFilename}, falling back to H.264. Error: ${svtError.message}`);
      }
    }

    // H.264 encoding (final fallback)
