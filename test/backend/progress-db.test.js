...
describe('Progress Database Functions', () => {
  let db;
  const testDbPath = path.join(__dirname, '..', 'test-progress-db.db');
  ...
  describe('Basic CRUD Operations', () => {
    ...
    test('should handle non-existent video progress gracefully', async () => {
      const progress = await dbModule.getVideoProgress(db, 999, testSession);
      expect(progress).toBe(undefined); // SQLite returns undefined for non-existent records
    });
